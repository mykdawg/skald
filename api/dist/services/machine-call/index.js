...
async function callMachine({ name }) {
    try {
        ;
        const helloProto = loadPackageDefinition(packageDefinition).hello;
        const client = new helloProto.Greeter(AGENT_WORKFLOWS_HOST, grpc.credentials.createInsecure());
        await client.sayHello({ name }, async function (err, response) {
            (0, message_1.publish)('story', response.message);
        });
